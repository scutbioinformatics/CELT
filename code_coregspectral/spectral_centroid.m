function [MA_nmi_value,MA_ACC,MA_AR,MA_f,MA_p,MA_r,MA_Purity,L_ustar]  = spectral_centroid(X1,X2,numClust,sigma1,sigma2,lambda1,lambda2,truth2,numiter)
% INPUT:
% OUTPUT:
    ...
    for j=1:kmeans_avg_iter
        ...
        [Fj(j),Pj(j),Rj(j)] = compute_f(truth2,C); 
        [Aj nmi_j(j) avgent_j(j)] = compute_nmi(truth2,C);
        [ARj(j),RIj(j),MIj(j),HIj(j)]=RandIndex(truth2,C);
    end
