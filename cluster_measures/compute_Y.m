...
% else
    ...
    eigvalue = eigvalue(index);  %%% eigvalue from big to small
    Y = Y(:,index);
...
nGotDim = length(eigvalue);  %%% nGotDim=min(ReducedDim, length(eigvalue))
    
idx = 1; 
while(abs(eigvalue(idx)-1) < 1e-12)  %%%value close to 1
    idx = idx + 1;
    if idx > nGotDim
        break;
    end 
end
idx = idx - 1;  %%% idx = min(nGotDim, >1e-12)
